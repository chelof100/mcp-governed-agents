...
We validate T9.3 empirically through three sub-experiments under
controlled multi-hop simulation aligned with current A2A draft
semantics \citep{google2025a2a}. All delegation chains are
instantiated within a single process using the proxy's
\texttt{delegation\_chain} and \texttt{agent\_id} parameters;
cross-vendor interoperability with live, heterogeneous A2A
ecosystems is not claimed---the results establish formal and
implementation feasibility pending ecosystem-wide protocol
stabilisation.
...
\begin{enumerate}[leftmargin=2em, topsep=2pt, itemsep=2pt]
  ...
  \item \textbf{APBRequired prioritises safety over liveness.}
        The current protocol deliberately blocks the tool call until
        the client provides a signed APB: \emph{no authority, no
        action}. This conservative posture is intentional and
        appropriate for compliance-critical, financial, and
        infrastructure deployments where silent execution is
        unacceptable. The operational cost is that a non-responsive
        client suspends session progress until explicit authority
        resolution. Paper~10 will introduce timeout semantics,
        escrow-state serialisation, and asynchronous approval queues
        as a controlled relaxation of this safety bound while
        preserving the core accountability guarantee.
